import { Q as client_only_default, H as useI18n, W as nuxt_link_default, s as packageRoute, n as DateTime_default, i as Base_default, a as _plugin_vue_export_helper_default } from './server.mjs';
import { u as useNumberFormatter } from './useNumberFormatter-B-AHKObJ.mjs';
import { u as useMarkdown, P as ProvenanceBadge_default } from './useMarkdown-DuX2wexs.mjs';
import { defineComponent, useTemplateRef, useModel, computed, shallowRef, watch, nextTick, unref, withCtx, createVNode, openBlock, createBlock, Fragment, renderList, mergeModels, mergeProps, toValue, resolveDynamicComponent, createTextVNode, toDisplayString, createCommentVNode, withModifiers, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderList, ssrInterpolate, ssrRenderSlot, ssrRenderClass, ssrRenderAttr, ssrRenderVNode } from 'vue/server-renderer';
import { aY as DEFAULT_COLUMNS, aV as parseSortOption } from '../nitro/nitro.mjs';
...
var TableRow_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			}, _attrs))} data-v-a30c6a63><td class="py-2 px-3" data-v-a30c6a63>`);
			...
			_push(`</td>`);
			if (isColumnVisible("version")) _push(`<td class="py-2 px-3 font-mono text-xs text-fg-subtle" data-v-a30c6a63><span dir="ltr" data-v-a30c6a63>${ssrInterpolate(unref(pkg).version)}</span></td>`);
			else _push(`<!---->`);
			if (isColumnVisible("description")) _push(`<td class="py-2 px-3 text-sm text-fg-muted max-w-xs truncate" data-v-a30c6a63>${ssrInterpolate(unref(pkg).description || "-")}</td>`);
			else _push(`<!---->`);
			if (isColumnVisible("downloads")) _push(`<td class="py-2 px-3 font-mono text-xs text-fg-muted text-end tabular-nums" data-v-a30c6a63>${ssrInterpolate(formatDownloads(__props.result.downloads?.weekly))}</td>`);
			else _push(`<!---->`);
			if (isColumnVisible("updated")) {
				_push(`<td class="py-2 px-3 font-mono text-end text-xs text-fg-muted" data-v-a30c6a63>`);
				...
				else _push(`<span data-v-a30c6a63>-</span>`);
				_push(`</td>`);
			} else _push(`<!---->`);
			if (isColumnVisible("maintainers")) {
				_push(`<td class="py-2 px-3 text-sm text-fg-muted text-end" data-v-a30c6a63>`);
				if (unref(pkg).maintainers?.length) {
					_push(`<span${ssrRenderAttr("title", unref(pkg).maintainers.length > 3 ? unref(allMaintainersText) : void 0)} class="${ssrRenderClass({ "cursor-help": unref(pkg).maintainers.length > 3 })}" data-v-a30c6a63><!--[-->`);
					ssrRenderList(unref(pkg).maintainers.slice(0, 3), (maintainer, idx) => {
						...
						if (idx < Math.min(unref(pkg).maintainers.length, 3) - 1) _push(`<span data-v-a30c6a63>, </span>`);
						else _push(`<!---->`);
						_push(`<!--]-->`);
					});
					_push(`<!--]-->`);
					if (unref(pkg).maintainers.length > 3) _push(`<span class="text-fg-subtle" data-v-a30c6a63> +${ssrInterpolate(unref(pkg).maintainers.length - 3)}</span>`);
					else _push(`<!---->`);
					_push(`</span>`);
				} else _push(`<span class="text-fg-subtle" data-v-a30c6a63>-</span>`);
				_push(`</td>`);
			} else _push(`<!---->`);
			if (isColumnVisible("keywords")) {
				_push(`<td class="py-2 px-3 text-end" data-v-a30c6a63>`);
				if (unref(pkg).keywords?.length) {
					_push(`<div class="relative z-10 flex flex-wrap gap-1 justify-end"${ssrRenderAttr("aria-label", _ctx.$t("package.card.keywords"))} data-v-a30c6a63><!--[-->`);
					...
					_push(`<!--]-->`);
					if (unref(pkg).keywords.length > 3) _push(`<span class="text-fg-subtle text-xs"${ssrRenderAttr("title", unref(pkg).keywords.slice(3).join(", "))} data-v-a30c6a63> +${ssrInterpolate(unref(pkg).keywords.length - 3)}</span>`);
					else _push(`<!---->`);
					_push(`</div>`);
				} else _push(`<span class="text-fg-subtle" data-v-a30c6a63>-</span>`);
				_push(`</td>`);
			} else _push(`<!---->`);
			if (isColumnVisible("qualityScore")) _push(`<td class="py-2 px-3 font-mono text-xs text-fg-muted text-end tabular-nums" data-v-a30c6a63>${ssrInterpolate(formatScore(unref(score)?.detail?.quality))}</td>`);
			else _push(`<!---->`);
			if (isColumnVisible("popularityScore")) _push(`<td class="py-2 px-3 font-mono text-xs text-fg-muted text-end tabular-nums" data-v-a30c6a63>${ssrInterpolate(formatScore(unref(score)?.detail?.popularity))}</td>`);
			else _push(`<!---->`);
			if (isColumnVisible("maintenanceScore")) _push(`<td class="py-2 px-3 font-mono text-xs text-fg-muted text-end tabular-nums" data-v-a30c6a63>${ssrInterpolate(formatScore(unref(score)?.detail?.maintenance))}</td>`);
			else _push(`<!---->`);
			if (isColumnVisible("combinedScore")) _push(`<td class="py-2 px-3 font-mono text-xs text-fg-muted text-end tabular-nums" data-v-a30c6a63>${ssrInterpolate(formatScore(unref(score)?.final))}</td>`);
			else _push(`<!---->`);
			if (isColumnVisible("security")) {
				_push(`<td class="py-2 px-3" data-v-a30c6a63>`);
				if (__props.result.flags?.insecure) _push(`<span class="text-syntax-kw" data-v-a30c6a63><span class="i-carbon-warning w-4 h-4" aria-hidden="true" data-v-a30c6a63></span><span class="sr-only" data-v-a30c6a63>${ssrInterpolate(_ctx.$t("filters.table.security_warning"))}</span></span>`);
				else if (__props.result.flags !== void 0) _push(`<span class="text-provider-nuxt" data-v-a30c6a63><span class="i-carbon-checkmark w-4 h-4" aria-hidden="true" data-v-a30c6a63></span><span class="sr-only" data-v-a30c6a63>${ssrInterpolate(_ctx.$t("filters.table.secure"))}</span></span>`);
				else _push(`<span class="text-fg-subtle" data-v-a30c6a63> - </span>`);
				_push(`</td>`);
			} else _push(`<!---->`);
			_push(`</tr>`);
...
var TableRow_default = /* @__PURE__ */ Object.assign(_plugin_vue_export_helper_default(TableRow_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-a30c6a63"]]), { __name: "PackageTableRow" });
...
//# sourceMappingURL=List-C77sESZJ.mjs.map
