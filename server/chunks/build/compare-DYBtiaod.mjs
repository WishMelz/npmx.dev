import { H as useI18n, lt as useRouter$1, nt as useSeoMeta$1, i as Base_default, rt as useRoute, it as useRouter, k as tryOnScopeDispose, a as _plugin_vue_export_helper_default, j as useTimeoutFn, I as Base_default$2, s as packageRoute, M as Base_default$1, n as DateTime_default } from './server.mjs';
import { u as useCanGoBack } from './useCanGoBack-DH1Opmzd.mjs';
import { S as Static_default, A as App_default } from './App-cOlf6p32.mjs';
import { D as DownloadAnalytics_default } from './DownloadAnalytics-BykjdyXg.mjs';
...
import { u as useSearch } from './useSearch-DuwuODWO.mjs';
import { defineComponent, computed, mergeProps, unref, isRef, withCtx, createTextVNode, toDisplayString, openBlock, createBlock, Fragment, renderList, customRef, toValue, nextTick, watch, shallowRef, readonly, useModel, createVNode, createCommentVNode, mergeModels, resolveComponent, useSSRContext } from 'vue';
...
import { b4 as DEFAULT_FACETS, b5 as ALL_FACETS, b6 as FACET_INFO, b7 as CATEGORY_ORDER, b8 as FACETS_BY_CATEGORY } from '../nitro/nitro.mjs';
	...
	setup(__props) {
		...
		function handleKeydown(e) {
			...
			if (e.key === "Enter" && inputValueTrim) {
				...
				else if (hasMatchInPackages) addPackage(inputValueTrim);
			}
		}
				...
				ssrRenderList(packages.value, (pkg) => {
					_push(ssrRenderComponent(_component_TagStatic, { key: pkg }, {
						default: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) {
								if (pkg === unref("__no_dependency__")) _push(`<span class="text-sm text-accent italic flex items-center gap-1.5"${_scopeId}><span class="i-carbon:clean w-3.5 h-3.5" aria-hidden="true"${_scopeId}></span> ${ssrInterpolate(_ctx.$t("compare.no_dependency.label"))}</span>`);
								...
								class: "i-carbon:clean w-3.5 h-3.5",
								"aria-hidden": "true"
			...
			if (packages.value.length < unref(maxPackages)) {
				...
				if (unref(isInputFocused) && (unref(filteredResults).length > 0 || unref(isSearching) || unref(showNoDependencyOption))) {
					...
					if (unref(showNoDependencyOption)) _push(ssrRenderComponent(_component_ButtonBase, {
						...
						default: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) _push(`<span class="text-sm text-accent italic flex items-center gap-2 block"${_scopeId}><span class="i-carbon:clean w-4 h-4" aria-hidden="true"${_scopeId}></span> ${ssrInterpolate(_ctx.$t("compare.no_dependency.typeahead_title"))}</span><span class="text-xs text-fg-muted truncate mt-0.5"${_scopeId}>${ssrInterpolate(_ctx.$t("compare.no_dependency.typeahead_description"))}</span>`);
							else return [createVNode("span", { class: "text-sm text-accent italic flex items-center gap-2 block" }, [createVNode("span", {
								class: "i-carbon:clean w-4 h-4",
								"aria-hidden": "true"
						...
						_push(ssrRenderComponent(_component_ButtonBase, {
							...
							default: withCtx((_, _push, _parent, _scopeId) => {
								if (_push) {
									...
									if (result.description) _push(`<span class="text-xs text-fg-muted truncate mt-0.5"${_scopeId}>${ssrInterpolate(result.description)}</span>`);
									else _push(`<!---->`);
								...
								}, toDisplayString(result.description), 1)) : createCommentVNode("", true)];
							}),
							_: 2
						}, _parent));
					});
...
var ComparisonGrid_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "overflow-x-auto" }, _attrs))} data-v-bd58a3f6><div class="${ssrRenderClass([[unref(totalColumns) === 4 ? "min-w-[800px]" : "min-w-[600px]", `columns-${unref(totalColumns)}`], "comparison-grid"])}" style="${ssrRenderStyle({ "--columns": unref(totalColumns) })}" data-v-bd58a3f6><div class="comparison-header" data-v-bd58a3f6><div class="comparison-label" data-v-bd58a3f6></div><!--[-->`);
			ssrRenderList(__props.columns, (col) => {
				_push(`<div class="comparison-cell comparison-cell-header" data-v-bd58a3f6><span class="inline-flex items-center gap-1.5 truncate" data-v-bd58a3f6>`);
				...
				if (col.replacement) _push(ssrRenderComponent(_component_TooltipApp, {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`<span class="i-carbon:idea w-3.5 h-3.5 text-amber-500 shrink-0 cursor-help" role="img"${ssrRenderAttr("aria-label", unref($t)("package.replacement.title"))} data-v-bd58a3f6${_scopeId}></span>`);
			...
			if (__props.showNoDependency) {
				_push(`<div class="comparison-cell comparison-cell-header comparison-cell-nodep" data-v-bd58a3f6><span class="inline-flex items-center gap-1.5 text-sm font-medium text-accent italic truncate" data-v-bd58a3f6>${ssrInterpolate(unref($t)("compare.no_dependency.label"))} `);
				_push(ssrRenderComponent(_component_TooltipApp, {
					...
					content: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) {
							_push(`<p class="text-sm font-medium text-fg mb-1" data-v-bd58a3f6${_scopeId}>${ssrInterpolate(unref($t)("compare.no_dependency.tooltip_title"))}</p><p class="text-xs text-fg-muted" data-v-bd58a3f6${_scopeId}>`);
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`<span class="i-carbon:idea w-3.5 h-3.5 text-amber-500 shrink-0 cursor-help" role="img"${ssrRenderAttr("aria-label", unref($t)("compare.no_dependency.tooltip_title"))} data-v-bd58a3f6${_scopeId}></span>`);
						else return [createVNode("span", {
							class: "i-carbon:idea w-3.5 h-3.5 text-amber-500 shrink-0 cursor-help",
							role: "img",
...
var ComparisonGrid_default = /* @__PURE__ */ Object.assign(_plugin_vue_export_helper_default(ComparisonGrid_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-bd58a3f6"]]), { __name: "CompareComparisonGrid" });
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(canCompare)) {
				...
				if (unref(status) === "pending" && (!unref(packagesData) || unref(packagesData).every((p) => p === null))) {
					...
					_push(`<!--]--></div><h2 id="comparison-heading" class="text-xs text-fg-subtle uppercase tracking-wider mb-4 mt-10">${ssrInterpolate(unref($t)("compare.facets.trends.title"))}</h2>`);
...
//# sourceMappingURL=compare-DYBtiaod.mjs.map
