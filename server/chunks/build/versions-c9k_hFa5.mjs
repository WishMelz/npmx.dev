import { compare, validRange, satisfies } from 'semver';

//#region app/utils/versions.ts
/**
...
function getTagPriority(tag) {
	...
	for (const [key, priority] of Object.entries(TAG_PRIORITY)) if (tag.toLowerCase().includes(key)) return priority;
	return Infinity;
}
/**
...
*/
function compareTagRows(rowA, rowB, versionTimes) {
	...
	return (versionTimes[rowB.version] ?? "").localeCompare(timeA);
}
/**
...
* @returns Negative/zero/positive comparator value
*/
function compareVersionGroupKeys(a, b) {
...
*/
function buildTaggedVersionRows(distTags) {
	const versionToTags = buildVersionToTagsMap(distTags);
	return Array.from(versionToTags.entries()).map(([version, tags]) => ({
		id: `version:${version}`,
		primaryTag: tags[0],
		tags,
		version
...
export { getVersionGroupKey as a, buildVersionToTagsMap as b, getVersionGroupLabel as c, filterExcludedTags as d, buildTaggedVersionRows as e, filterVersions as f, getPrereleaseChannel as g, compareTagRows as h, isSameVersionGroup as i, compareVersionGroupKeys as j };
//# sourceMappingURL=versions-c9k_hFa5.mjs.map
