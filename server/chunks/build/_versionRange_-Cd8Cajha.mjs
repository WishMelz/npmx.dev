import { d as useRoute, i as useRouter, a3 as diffRoute, a as useSeoMeta$1, n as nuxt_link_default, w as packageRoute, o as client_only_default, _ as _plugin_vue_export_helper_default, u as useI18n, L as onClickOutside } from './server-placeholder-C9fYItBT.mjs';
import { a as useFetch } from './fetch-CVxFI0ck.mjs';
import { u as usePackage } from './usePackage-RzwDULD4.mjs';
import { b as useBytesFormatter } from './useNumberFormatter-6MIdB6Qd.mjs';
import { H as Header_default, V as VersionSelector_default } from './Header-ca9VNABN.mjs';
import { T as Toggle_server_default } from './Toggle.server-QMbdN081.mjs';
import { defineComponent, computed, ref, resolveComponent, mergeProps, unref, withCtx, createVNode, toDisplayString, createTextVNode, isRef, useModel, mergeModels, useTemplateRef, watchEffect, provide, inject, resolveDynamicComponent, openBlock, createBlock, Fragment, renderList, useSSRContext } from 'vue';
...
//#region app/components/Diff/FileTree.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region app/components/Diff/FileTree.vue
...
//#endregion
//#region app/components/Diff/SidebarPanel.vue?vue&type=script&setup=true&lang.ts
var SidebarPanel_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (__props.compare.dependencyChanges.length > 0) _push(`<span class="text-fg-muted">${ssrInterpolate(_ctx.$t("compare.deps_count", { count: __props.compare.dependencyChanges.length }, __props.compare.dependencyChanges.length))}</span>`);
			else _push(`<!---->`);
			_push(`</div></div></div>`);
...
//#endregion
//#region app/components/Diff/SidebarPanel.vue
...
//#endregion
//#region app/components/Diff/Line.vue?vue&type=script&setup=true&lang.ts
var Line_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			}, _attrs))} data-v-430081df><td class="${ssrRenderClass(unref(borderClasses))}" data-v-430081df></td><td class="tabular-nums text-center opacity-50 px-2 text-xs select-none w-12 shrink-0" data-v-430081df>${ssrInterpolate(__props.line.type === "delete" ? "–" : unref(lineNumberNew))}</td><td class="${ssrRenderClass(unref(contentClasses))}" data-v-430081df>`);
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						ssrRenderList(unref(renderedSegments), (seg, i) => {
							...
							})}" data-v-430081df${_scopeId}>${seg.html ?? ""}</span>`);
						});
						_push(`<!--]-->`);
...
//#endregion
//#region app/components/Diff/Line.vue
...
var Line_default = /* @__PURE__ */ Object.assign(_plugin_vue_export_helper_default(Line_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-430081df"]]), { __name: "DiffLine" });
//#endregion
//#region app/components/Diff/Hunk.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region app/components/Diff/Hunk.vue
...
//#endregion
//#region app/components/Diff/SkipBlock.vue?vue&type=script&setup=true&lang.ts
...
//#endregion
//#region app/components/Diff/SkipBlock.vue
...
//#endregion
//#region app/components/Diff/Table.vue?vue&type=script&setup=true&lang.ts
var Table_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<table${ssrRenderAttrs(mergeProps({ class: "diff-table shiki font-mono text-sm w-full m-0 border-separate border-0 outline-none overflow-x-auto border-spacing-0" }, _attrs))} data-v-0d3f1521><tbody class="w-full box-border" data-v-0d3f1521><!--[-->`);
...
//#endregion
//#region app/components/Diff/Table.vue
...
var Table_default = /* @__PURE__ */ Object.assign(_plugin_vue_export_helper_default(Table_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-0d3f1521"]]), { __name: "DiffTable" });
//#endregion
//#region app/components/Diff/ViewerPanel.vue?vue&type=script&setup=true&lang.ts
...
var ViewerPanel_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const isFilesTooLarge = computed(() => {
			...
			return newSize !== void 0 && newSize > MAX_FILE_SIZE || oldSize !== void 0 && oldSize > MAX_FILE_SIZE;
		});
		const apiUrl = computed(() => isFilesTooLarge.value ? null : `/api/registry/compare-file/${props.packageName}/v/${props.fromVersion}...${props.toVersion}/${props.file.path}`);
		const { data: diff, status, error: loadError } = useFetch(() => apiUrl.value, {
			query: computed(() => ({
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "h-full flex flex-col bg-bg" }, _attrs))} data-v-85f5c163><div class="flex flex-wrap items-center justify-between gap-3 px-4 py-3 bg-bg-subtle border-b border-border shrink-0" data-v-85f5c163><div class="flex items-center gap-3 min-w-0 flex-1" data-v-85f5c163><span class="${ssrRenderClass(["w-4 h-4 shrink-0", __props.file.type === "added" ? "i-lucide:plus text-green-500" : __props.file.type === "removed" ? "i-lucide:minus text-red-500" : "i-lucide:pen text-yellow-500"])}" data-v-85f5c163></span><span class="font-mono text-sm truncate max-w-[65vw] sm:max-w-none" data-v-85f5c163>${ssrInterpolate(__props.file.path)}</span>`);
			if (unref(diff)?.stats) {
				_push(`<!--[-->`);
				if (unref(diff).stats.additions > 0) _push(`<span class="text-xs text-green-500 font-mono shrink-0" data-v-85f5c163> +${ssrInterpolate(unref(diff).stats.additions)}</span>`);
				else _push(`<!---->`);
				if (unref(diff).stats.deletions > 0) _push(`<span class="text-xs text-red-500 font-mono shrink-0" data-v-85f5c163> -${ssrInterpolate(unref(diff).stats.deletions)}</span>`);
				else _push(`<!---->`);
				_push(`<!--]-->`);
			} else _push(`<!---->`);
			if (__props.file.oldSize || __props.file.newSize) {
				_push(`<span class="text-xs text-fg-subtle shrink-0" data-v-85f5c163>`);
				if (__props.file.type === "modified") _push(`<!--[-->${ssrInterpolate(unref(bytesFormatter).format(__props.file.oldSize ?? 0))} → ${ssrInterpolate(unref(bytesFormatter).format(__props.file.newSize ?? 0))}<!--]-->`);
				else if (__props.file.type === "added") _push(`<!--[-->${ssrInterpolate(unref(bytesFormatter).format(__props.file.newSize ?? 0))}<!--]-->`);
				else _push(`<!--[-->${ssrInterpolate(unref(bytesFormatter).format(__props.file.oldSize ?? 0))}<!--]-->`);
				_push(`</span>`);
			} else _push(`<!---->`);
			_push(`</div><div class="flex items-center gap-2 shrink-0" data-v-85f5c163><div class="relative" data-v-85f5c163><button type="button" class="${ssrRenderClass([{ "bg-bg-elevated text-fg": unref(showOptions) }, "px-2 py-1 text-xs text-fg-muted hover:text-fg bg-bg-muted border border-border rounded transition-colors flex items-center gap-1.5"])}" data-v-85f5c163><span class="i-lucide:settings w-3.5 h-3.5" data-v-85f5c163></span> ${ssrInterpolate(_ctx.$t("compare.options"))} <span class="${ssrRenderClass([{ "rotate-180": unref(showOptions) }, "i-lucide:chevron-down w-3 h-3 transition-transform"])}" data-v-85f5c163></span></button>`);
			if (unref(showOptions)) {
				_push(`<div class="absolute inset-ie-0 top-full mt-2 z-20 p-4 bg-bg-elevated border border-border rounded-xl shadow-2xl overflow-auto" style="${ssrRenderStyle({
					...
					maxHeight: "70vh"
				})}" data-v-85f5c163><div class="flex flex-col gap-2" data-v-85f5c163>`);
				_push(ssrRenderComponent(_component_SettingsToggle, {
					label: _ctx.$t("compare.merge_modified_lines"),
				...
				_push(ssrRenderComponent(_component_SettingsToggle, {
					label: _ctx.$t("compare.word_wrap"),
					modelValue: unref(wordWrap),
					"onUpdate:modelValue": ($event) => isRef(wordWrap) ? wordWrap.value = $event : null
				}, null, _parent));
				_push(`<div class="${ssrRenderClass([unref(mergeModifiedLines) ? "opacity-100" : "opacity-0", "flex flex-col gap-2 transition-opacity duration-150"])}" data-v-85f5c163><div class="sr-only" data-v-85f5c163><label for="change-ratio" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.change_ratio"))}</label></div><div class="${ssrRenderClass([{ "is-disabled": !unref(mergeModifiedLines) }, "slider-shell w-full min-w-0"])}" data-v-85f5c163><div class="slider-labels" data-v-85f5c163><span class="slider-label" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.change_ratio"))}</span><span class="slider-value tabular-nums" data-v-85f5c163>${ssrInterpolate(unref(maxChangeRatio).toFixed(2))}</span></div><div class="slider-track" data-v-85f5c163><!--[-->`);
				ssrRenderList(unref(changeRatioMarks), (mark) => {
					_push(`<div class="slider-mark" style="${ssrRenderStyle({ left: `calc(${mark}% - 11px)` })}" data-v-85f5c163></div>`);
				});
				_push(`<!--]--><div class="slider-range" style="${ssrRenderStyle({ width: `${unref(changeRatioPercent)}%` })}" data-v-85f5c163></div></div><input id="change-ratio"${ssrRenderAttr("value", unref(maxChangeRatio))} type="range" min="0" max="1" step="0.01"${ssrIncludeBooleanAttr(!unref(mergeModifiedLines)) ? " disabled" : ""} class="slider-input" data-v-85f5c163></div><div class="sr-only" data-v-85f5c163><label for="diff-distance" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.diff_distance"))}</label></div><div class="${ssrRenderClass([{ "is-disabled": !unref(mergeModifiedLines) }, "slider-shell w-full min-w-0"])}" data-v-85f5c163><div class="slider-labels" data-v-85f5c163><span class="slider-label" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.diff_distance"))}</span><span class="slider-value tabular-nums" data-v-85f5c163>${ssrInterpolate(unref(maxDiffDistance))}</span></div><div class="slider-track" data-v-85f5c163><!--[-->`);
				ssrRenderList(unref(diffDistanceMarks), (mark) => {
					_push(`<div class="slider-mark" style="${ssrRenderStyle({ left: `calc(${mark}% - 11px)` })}" data-v-85f5c163></div>`);
				});
				_push(`<!--]--><div class="slider-range" style="${ssrRenderStyle({ width: `${unref(diffDistancePercent)}%` })}" data-v-85f5c163></div></div><input id="diff-distance"${ssrRenderAttr("value", unref(maxDiffDistance))} type="range" min="1" max="60" step="1"${ssrIncludeBooleanAttr(!unref(mergeModifiedLines)) ? " disabled" : ""} class="slider-input" data-v-85f5c163></div><div class="sr-only" data-v-85f5c163><label for="char-edits" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.char_edits"))}</label></div><div class="${ssrRenderClass([{ "is-disabled": !unref(mergeModifiedLines) }, "slider-shell w-full min-w-0"])}" data-v-85f5c163><div class="slider-labels" data-v-85f5c163><span class="slider-label" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.char_edits"))}</span><span class="slider-value tabular-nums" data-v-85f5c163>${ssrInterpolate(unref(inlineMaxCharEdits))}</span></div><div class="slider-track" data-v-85f5c163><!--[-->`);
				ssrRenderList(unref(charEditMarks), (mark) => {
					_push(`<div class="slider-mark" style="${ssrRenderStyle({ left: `calc(${mark}% - 11px)` })}" data-v-85f5c163></div>`);
				});
				_push(`<!--]--><div class="slider-range" style="${ssrRenderStyle({ width: `${unref(charEditPercent)}%` })}" data-v-85f5c163></div></div><input id="char-edits"${ssrRenderAttr("value", unref(inlineMaxCharEdits))} type="range" min="0" max="10" step="1"${ssrIncludeBooleanAttr(!unref(mergeModifiedLines)) ? " disabled" : ""} class="slider-input" data-v-85f5c163></div></div></div></div>`);
			} else _push(`<!---->`);
			_push(`</div>`);
			if (__props.file.type !== "removed") _push(ssrRenderComponent(_component_NuxtLink, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`${ssrInterpolate(_ctx.$t("compare.view_file"))}`);
					else return [createTextVNode(toDisplayString(_ctx.$t("compare.view_file")), 1)];
				}),
				_: 1
			}, _parent));
			else _push(`<!---->`);
			_push(`</div></div><div class="flex-1 overflow-auto relative" data-v-85f5c163>`);
			if (unref(isFilesTooLarge)) _push(`<div class="py-20 text-center" data-v-85f5c163><div class="i-lucide:file-text w-12 h-12 mx-auto text-fg-subtle mb-4" data-v-85f5c163></div><p class="text-fg-muted mb-2" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.file_too_large"))}</p><p class="text-fg-subtle text-sm mb-4" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.file_size_warning", { size: unref(bytesFormatter).format(Math.max(__props.file.newSize ?? 0, __props.file.oldSize ?? 0)) }))}</p></div>`);
			else if (unref(status) === "pending") _push(`<div class="py-12 text-center" data-v-85f5c163><div class="i-svg-spinners-ring-resize w-6 h-6 mx-auto text-fg-muted" data-v-85f5c163></div><p class="mt-2 text-sm text-fg-muted" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.loading_diff"))}</p></div>`);
			else if (unref(status) === "error") {
				_push(`<div class="py-8 text-center" data-v-85f5c163><span class="i-lucide:triangle-alert w-8 h-8 mx-auto text-fg-subtle mb-2 block" data-v-85f5c163></span><p class="text-fg-muted text-sm mb-2" data-v-85f5c163>${ssrInterpolate(unref(loadError)?.message || _ctx.$t("compare.loading_diff_error"))}</p><div class="flex items-center justify-center gap-2" data-v-85f5c163>`);
				if (__props.file.type !== "removed") _push(ssrRenderComponent(_component_NuxtLink, {
					to: getCodeUrl(__props.toVersion),
					class: "text-xs text-fg-muted hover:text-fg underline"
				}, {
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`${ssrInterpolate(_ctx.$t("compare.view_in_code_browser"))}`);
						else return [createTextVNode(toDisplayString(_ctx.$t("compare.view_in_code_browser")), 1)];
					}),
					_: 1
				}, _parent));
				else _push(`<!---->`);
				_push(`</div></div>`);
			} else if (unref(diff) && unref(diff).hunks.length === 0) _push(`<div class="py-8 text-center text-fg-muted text-sm" data-v-85f5c163>${ssrInterpolate(_ctx.$t("compare.no_content_changes"))}</div>`);
...
//#endregion
//#region app/components/Diff/ViewerPanel.vue
...
var ViewerPanel_default = /* @__PURE__ */ Object.assign(_plugin_vue_export_helper_default(ViewerPanel_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-85f5c163"]]), { __name: "DiffViewerPanel" });
//#endregion
//#region app/pages/diff/[[org]]/[packageName]/v/[versionRange].vue?vue&type=script&setup=true&lang.ts
var _versionRange__vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const normalizeRoutePath = (routeLocation) => {
			return router.resolve(routeLocation).href.replace(/%7B/g, "{").replace(/%7D/g, "}");
		};
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<main${ssrRenderAttrs(mergeProps({ class: "flex-1 flex flex-col min-h-0" }, _attrs))}>`);
			_push(ssrRenderComponent(_component_PackageHeader, {
				pkg: unref(pkg),
				"resolved-version": unref(fromVersion),
				"display-version": unref(pkg)?.requestedVersion,
				"latest-version": unref(latestVersionDetailed),
				"version-url-pattern": unref(fromVersionUrlPattern),
				page: "diff"
			}, null, _parent));
			if (!unref(versionRange)) {
				_push(`<div class="container py-20 text-center">`);
				_push(ssrRenderComponent(_component_i18n_t, {
					keypath: "compare.version_invalid_url_format.hint",
					tag: "p",
					class: "text-fg-muted mb-4"
				}, {
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`<code class="font-mono text-sm"${_scopeId}>/diff/${ssrInterpolate(unref(packageName))}/v/${ssrInterpolate(_ctx.$t("compare.version_invalid_url_format.from_version"))}...${ssrInterpolate(_ctx.$t("compare.version_invalid_url_format.to_version"))}</code>`);
						else return [createVNode("code", { class: "font-mono text-sm" }, "/diff/" + toDisplayString(unref(packageName)) + "/v/" + toDisplayString(_ctx.$t("compare.version_invalid_url_format.from_version")) + "..." + toDisplayString(_ctx.$t("compare.version_invalid_url_format.to_version")), 1)];
					}),
					_: 1
				}, _parent));
				_push(ssrRenderComponent(_component_NuxtLink, {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`${ssrInterpolate(_ctx.$t("compare.version_back_to_package"))}`);
						else return [createTextVNode(toDisplayString(_ctx.$t("compare.version_back_to_package")), 1)];
					}),
					_: 1
				}, _parent));
				_push(`</div>`);
			} else if (unref(compareStatus) === "pending") _push(`<div class="container py-20 text-center"><div class="i-svg-spinners-ring-resize w-8 h-8 mx-auto text-fg-muted"></div><p class="mt-4 text-fg-muted">${ssrInterpolate(_ctx.$t("compare.comparing_versions_label"))}</p></div>`);
			else if (unref(compareStatus) === "error") {
				_push(`<div class="container py-20 text-center" role="alert"><p class="text-fg-muted mb-4">${ssrInterpolate(_ctx.$t("compare.version_error_message"))}</p>`);
				_push(ssrRenderComponent(_component_NuxtLink, {
					to: ("packageRoute" in _ctx ? _ctx.packageRoute : unref(packageRoute))(unref(packageName)),
					class: "btn"
				}, {
				...
				if (unref(pkg)?.versions && unref(pkg)?.["dist-tags"]) {
					_push(`<div class="px-3 py-2 border-b border-border"><p class="text-xs font-medium text-fg mb-1 flex items-center gap-1.5"><span class="block i-lucide-git-compare-arrows w-3.5 h-3.5"></span> ${ssrInterpolate(_ctx.$t("compare.version_selector_title"))}</p>`);
					_push(ssrRenderComponent(_component_VersionSelector, {
...
//#endregion
//#region app/pages/diff/[[org]]/[packageName]/v/[versionRange].vue
...
//# sourceMappingURL=_versionRange_-Cd8Cajha.mjs.map
