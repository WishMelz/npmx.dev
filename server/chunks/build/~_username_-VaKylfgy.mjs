import { ct as useRoute$1, lt as useRouter$1, nt as useSeoMeta$1, I as Base_default$2, u as useSearchProvider, ut as useNuxtApp, Z as useLazyAsyncData, H as useI18n, M as Base_default$1 } from './server.mjs';
import { d as defineOgImageComponent } from './defineOgImageComponent-DtDCPnhJ.mjs';
import { L as LoadingSpinner_default, u as useAlgoliaSearch, e as emptySearchResponse } from './LoadingSpinner-er2gks8E.mjs';
import { F as Field_default } from './Field-CwiCxWgG.mjs';
import { L as List_default } from './List-C77sESZJ.mjs';
import { A as Avatar_default } from './Avatar-B6nr6vA4.mjs';
...
import { aC as normalizeSearchParam } from '../nitro/nitro.mjs';
import 'devalue';
import 'lru-cache';
import '@unocss/core';
...
var ListControls_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<!--[--><div class="flex flex-col sm:flex-row gap-3 mb-6"><div class="flex-1 relative"><label for="package-filter" class="sr-only">${ssrInterpolate(unref($t)("package.list.filter_label"))}</label><div class="absolute h-full w-10 flex items-center justify-center text-fg-subtle pointer-events-none" aria-hidden="true"><div class="i-carbon:search w-4 h-4"></div></div>`);
...
function useUserPackages(username) {
	...
	const activeProvider = shallowRef(searchProvider.value);
	const cache = shallowRef(null);
	const isLoadingMore = shallowRef(false);
	const asyncData = useLazyAsyncData(() => `user-packages:${searchProvider.value}:${toValue(username)}`, async ({ $npmRegistry }, { signal }) => {
		...
		if (provider === "algolia") try {
			...
			if (user !== toValue(username) || provider !== searchProvider.value) return emptySearchResponse();
...
//# sourceMappingURL=~_username_-VaKylfgy.mjs.map
