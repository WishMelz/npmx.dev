import { k as useI18n, Y as useRoute$1, X as useRouter$1, a as useConnector, q as showError, H as useHead$1, U as useSeoMeta$1, z as client_only_default, j as nuxt_link_default, R as useLazyAsyncData, K as createError$1 } from './server.mjs';
import { u as useCachedFetch, N as NPM_REGISTRY, a as NPM_API } from './useCachedFetch-B1uvSFXX.mjs';
import { L as LoadingSpinner_default, e as emptySearchResponse, p as packumentToSearchResult } from './useNpmSearch-BEb17E8y.mjs';
...
import { ar as normalizeSearchParam, aL as mapWithConcurrency, aM as encodePackageName } from '../nitro/nitro.mjs';
import 'devalue';
import 'lru-cache';
import '@unocss/core';
...
async function fetchBulkDownloads(packageNames, options = {}) {
	...
	for (let i = 0; i < unscopedPackages.length; i += chunkSize) {
		...
		bulkPromises.push((async () => {
			try {
				const response = await $fetch(`${NPM_API}/downloads/point/last-week/${chunk.join(",")}`, options);
				for (const [name, data] of Object.entries(response)) if (data?.downloads !== void 0) downloads.set(name, data.downloads);
			} catch {}
		})());
	}
	const scopedBatchSize = 20;
	for (let i = 0; i < scopedPackages.length; i += scopedBatchSize) {
		...
		bulkPromises.push((async () => {
			const results = await Promise.allSettled(batch.map(async (name) => {
				...
				return {
					name,
					downloads: (await $fetch(`${NPM_API}/downloads/point/last-week/${encoded}`)).downloads
				};
			}));
			for (const result of results) if (result.status === "fulfilled" && result.value.downloads !== void 0) downloads.set(result.value.name, result.value.downloads);
		})());
	}
...
function useOrgPackages(orgName) {
	...
	return useLazyAsyncData(() => `org-packages:${toValue(orgName)}`, async (_nuxtApp, { signal }) => {
		const org = toValue(orgName);
		if (!org) return emptySearchResponse;
		let packageNames;
		try {
			const { data } = await cachedFetch(`${NPM_REGISTRY}/-/org/${encodeURIComponent(org)}/package`, { signal });
			packageNames = Object.keys(data);
		} catch (err) {
			if (err && typeof err === "object" && "statusCode" in err && err.statusCode === 404) throw createError$1({
				statusCode: 404,
		...
		const [packuments, downloads] = await Promise.all([(async () => {
			return (await mapWithConcurrency(packageNames, async (name) => {
				try {
					const { data: pkg } = await cachedFetch(`${NPM_REGISTRY}/${encodePackageName(name)}`, { signal });
					return pkg;
				} catch {
					return null;
				}
			}, 10)).filter((pkg) => pkg !== null && !!pkg["dist-tags"]);
		})(), fetchBulkDownloads(packageNames, { signal })]);
		const results = packuments.map((pkg) => packumentToSearchResult(pkg, downloads.get(pkg.name)));
		return {
			isStale: false,
			objects: results,
...
var __org__vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	__name: "@[org]",
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`</div><div class="ms-auto text-end"><nav aria-label="External links"><a${ssrRenderAttr("href", `https://www.npmjs.com/org/${unref(orgName)}`)} target="_blank" rel="noopener noreferrer" class="link-subtle font-mono text-sm inline-flex items-center gap-1.5"${ssrRenderAttr("title", unref($t)("common.view_on_npm"))}><span class="i-carbon:logo-npm w-4 h-4" aria-hidden="true"></span> npm </a></nav><p class="text-fg-subtle text-xs mt-1 flex items-center gap-1.5 justify-end cursor-help"${ssrRenderAttr("title", unref($t)("common.vanity_downloads_hint", { count: unref(filteredCount) }, unref(filteredCount)))}><span class="i-carbon:chart-line w-3.5 h-3.5" aria-hidden="true"></span><span class="font-mono">${ssrInterpolate(_ctx.$n(unref(totalWeeklyDownloads)))} ${ssrInterpolate(unref($t)("common.per_week"))}</span></p></div></div></header>`);
...
var _sfc_setup = __org__vue_vue_type_script_setup_true_lang_default.setup;
__org__vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/@[org].vue");
	return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
var __org__default = __org__vue_vue_type_script_setup_true_lang_default;

export { __org__default as default };
//# sourceMappingURL=_org_-j5oGJtrN.mjs.map
