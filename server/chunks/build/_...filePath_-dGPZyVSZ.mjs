import { u as useI18n, d as useRoute, a1 as useEventListener, P as useClipboard, Y as useHead$1, a as useSeoMeta$1, B as Base_default, w as packageRoute, n as nuxt_link_default, o as client_only_default, _ as _plugin_vue_export_helper_default, m as Base_default$1, $ as useState, F as navigateTo } from './server-placeholder-C9fYItBT.mjs';
...
import { H as Header_default } from './Header-ca9VNABN.mjs';
import { S as SkeletonBlock_default } from './SkeletonBlock-DdKo1i2Y.mjs';
import { R as Readme_default } from './Readme-DDyiOEPy.mjs';
import { defineComponent, computed, watch, shallowRef, nextTick, mergeProps, unref, withCtx, createTextVNode, toDisplayString, openBlock, createBlock, createVNode, useTemplateRef, createCommentVNode, useSSRContext } from 'vue';
...
var DEFAULT_ICON = "vscode-icons-default-file";
...
//#endregion
//#region app/composables/useFileTreeState.ts
...
//#endregion
//#region app/components/Code/FileTree.vue?vue&type=script&setup=true&lang.ts
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			ssrRenderList(__props.tree, (node) => {
				...
				if (node.type === "directory") {
					...
					default: withCtx((_, _push, _parent, _scopeId) => {
						if (_push) _push(`<svg class="size-[1em] me-1 shrink-0" viewBox="0 0 16 16" aria-hidden="true"${_scopeId}><use${ssrRenderAttr("href", `/file-tree-sprite.svg#${unref(getFileIcon)(node.name)}`)}${_scopeId}></use></svg><span class="truncate"${_scopeId}>${ssrInterpolate(node.name)}</span>`);
						else return [(openBlock(), createBlock("svg", {
...
//#endregion
//#region app/components/Code/FileTree.vue
...
//#endregion
//#region app/components/Code/Viewer.vue?vue&type=script&setup=true&lang.ts
var Viewer_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { emit: __emit }) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "code-viewer flex min-h-full max-w-full" }, _attrs))} data-v-95a23191><div class="line-numbers shrink-0 bg-bg-subtle border-ie border-solid border-border text-end select-none relative" style="${ssrRenderStyle({ "--line-digits": unref(lineDigits) })}" aria-hidden="true" data-v-95a23191><!--[-->`);
			ssrRenderList(unref(lineNumbers), (lineNum) => {
				_push(`<a${ssrRenderAttr("id", `L${lineNum}`)}${ssrRenderAttr("href", `#L${lineNum}`)} tabindex="-1" class="${ssrRenderClass([[isLineSelected(lineNum) ? "bg-yellow-500/20 text-fg" : "text-fg-subtle hover:text-fg-muted"], "line-number block px-3 py-0 font-mono text-sm leading-6 cursor-pointer transition-colors no-underline"])}" data-v-95a23191>${ssrInterpolate(lineNum)}</a>`);
			});
			_push(`<!--]--></div><div class="code-content flex-1 overflow-x-auto min-w-0" data-v-95a23191><div class="code-lines min-w-full w-fit" data-v-95a23191>${__props.html ?? ""}</div></div></div>`);
		};
	}
});
//#endregion
//#region app/components/Code/Viewer.vue
...
var Viewer_default = /* @__PURE__ */ Object.assign(_plugin_vue_export_helper_default(Viewer_vue_vue_type_script_setup_true_lang_default, [["__scopeId", "data-v-95a23191"]]), { __name: "CodeViewer" });
//#endregion
//#region app/components/Code/DirectoryListing.vue?vue&type=script&setup=true&lang.ts
			...
			if (unref(currentContents).length === 0) _push(`<div class="py-20 text-center text-fg-muted"><p>${ssrInterpolate(_ctx.$t("code.no_files"))}</p></div>`);
				...
				if (unref(parentPath) !== null) {
					...
					_push(ssrRenderComponent(_component_LinkBase, {
						...
						default: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) _push(`<svg class="size-[1em] me-1 shrink-0 text-yellow-600" viewBox="0 0 16 16" aria-hidden="true"${_scopeId}><use${ssrRenderAttr("href", `/file-tree-sprite.svg#${unref(ADDITIONAL_ICONS)["folder"]}`)}${_scopeId}></use></svg><span class="w-full flex justify-self-stretch items-center gap-2"${_scopeId}> .. </span>`);
							else return [(openBlock(), createBlock("svg", {
				...
				ssrRenderList(unref(currentContents), (node) => {
					...
					_push(ssrRenderComponent(_component_LinkBase, {
						...
						default: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) {
								_push(`<svg viewBox="0 0 16 16" class="${ssrRenderClass([node.type === "directory" ? "text-yellow-600" : void 0, "size-[1em] me-1 shrink-0"])}" aria-hidden="true"${_scopeId}><use${ssrRenderAttr("href", `/file-tree-sprite.svg#${node.type === "directory" ? unref(ADDITIONAL_ICONS)["folder"] : unref(getFileIcon)(node.name)}`)}${_scopeId}></use></svg><span class="w-full flex justify-self-stretch items-center gap-2"${_scopeId}><span class="flex-1"${_scopeId}>${ssrInterpolate(node.name)}</span>`);
...
//#endregion
//#region app/components/Code/DirectoryListing.vue
...
//#endregion
//#region app/utils/file-types.ts
var BINARY_MIME_PREFIXES = new Set([
	"image/",
	"audio/",
	"video/",
	"font/",
...
function isBinaryContentType(contentType) {
	for (const prefix of BINARY_MIME_PREFIXES) if (contentType.startsWith(prefix)) return true;
	return false;
}
//#endregion
//#region app/pages/package-code/[[org]]/[packageName]/v/[version]/[...filePath].vue?vue&type=script&setup=true&lang.ts
...
var ____filePath__vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		const { copied: fileContentCopied} = useClipboard({
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(_component_PackageHeader, {
				pkg: unref(pkg),
				"resolved-version": unref(version),
			...
			if (!unref(version)) {
				...
				_push(`<div class="flex flex-1" dir="ltr"><aside class="w-64 lg:w-72 border-ie border-border shrink-0 hidden md:block bg-bg-subtle sticky top-25 self-start h-[calc(100vh-7rem)] overflow-y-auto">`);
				...
				_push(`</aside><div class="flex-1 min-w-0 self-start"><div class="sticky z-5 top-25 bg-bg border-b border-border px-4 py-2 flex items-center justify-between gap-2 text-nowrap overflow-x-auto max-w-full"><div class="flex items-center gap-2">`);
				if (unref(fileContent)?.markdownHtml) {
					...
					ssrRenderList(markdownViewModes, (mode) => {
						_push(`<button role="tab" class="${ssrRenderClass([unref(markdownViewMode) === mode.key ? "bg-bg shadow text-fg border-border" : "text-fg-subtle hover:text-fg border-transparent", "px-2 py-1.5 font-mono text-xs rounded transition-colors duration-150 border border-solid focus-visible:outline-accent/70 inline-flex items-center gap-1.5"])}"${ssrRenderAttr("aria-selected", unref(markdownViewMode) === mode.key)}><span class="${ssrRenderClass([mode.icon, "inline-block h-3 w-3"])}" aria-hidden="true"></span> ${ssrInterpolate(mode.label)}</button>`);
					});
					_push(`<!--]--></div>`);
				} else _push(`<!---->`);
				_push(`<nav${ssrRenderAttr("aria-label", unref($t)("code.file_path"))} class="flex items-center gap-0.5 font-mono text-sm overflow-x-auto" dir="ltr">`);
				...
				_push(`<!--]--></nav></div>`);
				if (unref(isViewingFile) && !unref(isBinaryFile) && unref(fileContent)) {
					...
					if (!!unref(fileContent)?.content) _push(`<button type="button" class="px-2 py-1 font-mono text-xs text-fg-muted bg-bg-subtle border border-border rounded hover:text-fg hover:border-border-hover transition-colors inline-flex items-center gap-1 capitalize"><span class="${ssrRenderClass([unref(fileContentCopied) ? "i-lucide:check" : "i-lucide:file", "w-3 h-3"])}"></span> ${ssrInterpolate(unref(fileContentCopied) ? unref($t)("common.copied") : unref($t)("common.copy"))}</button>`);
					else _push(`<!---->`);
					_push(`<a${ssrRenderAttr("href", `https://cdn.jsdelivr.net/npm/${unref(packageName)}@${unref(version)}/${unref(filePath)}`)} target="_blank" rel="noopener noreferrer" class="px-2 py-1 font-mono text-xs text-fg-muted bg-bg-subtle border border-border rounded hover:text-fg hover:border-border-hover transition-colors inline-flex items-center gap-1">${ssrInterpolate(unref($t)("code.raw"))} <span class="i-lucide:external-link w-3 h-3"></span></a></div>`);
				} else _push(`<!---->`);
				_push(`</div>`);
				if (unref(isViewingFile) && !unref(isBinaryFile) && unref(fileContent)) {
					_push(`<!--[-->`);
					...
					_push(`<div class="sticky bottom-0 bg-bg border-t border-border px-4 py-1"><div class="flex items-center gap-3 text-sm justify-end"><span class="text-fg-muted" dir="auto">${ssrInterpolate(unref($t)("code.lines", { count: unref(fileContent).lines }))}</span>`);
					...
					_push(`</div></div><!--]-->`);
				} else if (unref(isViewingFile) && unref(isBinaryFile)) {
					_push(`<div class="py-20 text-center"><div class="i-lucide:binary w-12 h-12 mx-auto text-fg-subtle mb-4"></div><p class="text-fg-muted mb-2">${ssrInterpolate(unref($t)("code.binary_file"))}</p><p class="text-fg-subtle text-sm mb-4">${ssrInterpolate(unref($t)("code.binary_rendering_warning", { contentType: unref(fileContent)?.contentType ?? "unknown" }))}</p>`);
					_push(ssrRenderComponent(_component_LinkBase, {
						variant: "button-secondary",
...
//#endregion
//#region app/pages/package-code/[[org]]/[packageName]/v/[version]/[...filePath].vue
...
//# sourceMappingURL=_...filePath_-dGPZyVSZ.mjs.map
