...
var CallToAction_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(_attrs)}><h2 class="text-lg text-fg-subtle uppercase tracking-wider mb-6">${ssrInterpolate(unref($t)("about.get_involved.title"))}</h2><div class="grid gap-4 sm:grid-cols-3 sm:items-stretch sm:grid-rows-[auto,1fr,auto]"><!--[-->`);
			ssrRenderList(unref(socialLinks), (link) => {
				_push(`<div class="group relative grid gap-3 p-4 rounded-lg bg-bg-subtle hover:bg-bg-elevated border border-border hover:border-border-hover transition-all duration-200 sm:grid-rows-subgrid sm:row-span-3 focus-within:ring-2 focus-within:ring-accent/50"><h3 class="z-1 flex gap-2"><span class="${ssrRenderClass([link.icon, "shrink-0 mt-1 w-5 h-5 text-fg"])}" aria-hidden="true"></span><span class="font-medium text-fg">${ssrInterpolate(link.titleKey)}</span></h3><p class="z-1 text-sm text-fg-muted leading-relaxed">${ssrInterpolate(link.descriptionKey)}</p><a${ssrRenderAttr("href", link.href)} target="_blank" rel="noopener noreferrer" class="text-sm text-fg-muted group-hover:text-fg inline-flex items-center gap-1 mt-auto focus-visible:outline-none">${ssrInterpolate(link.ctaKey)} <span class="i-carbon:arrow-right rtl-flip w-3 h-3" aria-hidden="true"></span><span class="absolute z-0 inset-0" aria-hidden="true"></span></a></div>`);
			});
			_push(`<!--]--></div></div>`);
		};
	}
...
//# sourceMappingURL=CallToAction-C5cSDbAQ.mjs.map
