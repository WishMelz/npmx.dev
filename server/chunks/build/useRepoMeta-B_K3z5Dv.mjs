import { M as useFetch, R as useLazyAsyncData } from './server.mjs';
import { u as useCachedFetch } from './useCachedFetch-B1uvSFXX.mjs';
import { toValue, computed } from 'vue';
import { s as parseRepoUrl, b2 as GITLAB_HOSTS } from '../nitro/nitro.mjs';

function useResolvedVersion(packageName, requestedVersion) {
	return useFetch(() => {
		const version = toValue(requestedVersion);
		return version ? `https://npm.antfu.dev/${toValue(packageName)}@${version}` : `https://npm.antfu.dev/${toValue(packageName)}`;
	}, { transform: (data) => data.version }, "$PLE6xLrPwj");
}
...
//# sourceMappingURL=useRepoMeta-B_K3z5Dv.mjs.map
