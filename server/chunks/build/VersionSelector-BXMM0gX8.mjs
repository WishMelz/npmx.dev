import { o as onClickOutside, j as nuxt_link_default } from './server.mjs';
import { b as buildVersionToTagsMap, g as getPrereleaseChannel, i as isSameVersionGroup, a as getVersionGroupKey, c as getVersionGroupLabel } from './versions-BaJFJggK.mjs';
import { defineComponent, shallowRef, useTemplateRef, ref, computed, watch, mergeProps, unref, withCtx, createTextVNode, toDisplayString, createVNode, openBlock, createBlock, Fragment, renderList, createCommentVNode, useSSRContext } from 'vue';
...
var VersionSelector_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<span class="${ssrRenderClass([{ "rotate-180": unref(isOpen) }, "i-carbon:chevron-down w-3.5 h-3.5 transition-[transform] duration-200 motion-reduce:transition-none"])}" aria-hidden="true"></span></button>`);
			if (unref(isOpen)) {
				_push(`<div role="listbox" tabindex="0"${ssrRenderAttr("aria-activedescendant", unref(focusedIndex) >= 0 ? `version-${unref(flatItems)[unref(focusedIndex)]?.version?.version}` : void 0)} class="absolute top-full inset-is-0 mt-2 min-w-[220px] bg-bg-elevated border border-border rounded-lg shadow-lg z-50 py-1 max-h-[400px] overflow-y-auto overscroll-contain focus-visible:outline-none"><!--[-->`);
				ssrRenderList(unref(versionGroups), (group) => {
					_push(`<div><div${ssrRenderAttr("id", `version-${group.primaryVersion.version}`)} role="option"${ssrRenderAttr("aria-selected", group.primaryVersion.isCurrent)}${ssrRenderAttr("data-focused", unref(flatItems)[unref(focusedIndex)]?.groupId === group.id && unref(flatItems)[unref(focusedIndex)]?.type === "group")} class="${ssrRenderClass([[group.primaryVersion.isCurrent ? "text-fg bg-bg-muted" : "text-fg-muted", unref(flatItems)[unref(focusedIndex)]?.groupId === group.id && unref(flatItems)[unref(focusedIndex)]?.type === "group" ? "bg-bg-muted" : ""], "flex items-center gap-2 px-3 py-2 text-sm font-mono hover:bg-bg-muted transition-[color,background-color] focus-visible:outline-none cursor-pointer"])}">`);
					if (group.versions.length > 1 || !unref(hasLoadedAll)) {
						...
						if (group.isLoading) _push(`<span class="i-carbon:rotate-180 w-3 h-3 motion-safe:animate-spin" aria-hidden="true"></span>`);
						else _push(`<span class="${ssrRenderClass([group.isExpanded ? "i-carbon:chevron-down" : "i-carbon:chevron-right", "w-3 h-3 transition-transform duration-200 rtl-flip"])}" aria-hidden="true"></span>`);
						_push(`</button>`);
					...
					_push(ssrRenderComponent(_component_NuxtLink, {
						...
						default: withCtx((_, _push, _parent, _scopeId) => {
							if (_push) _push(`${ssrInterpolate(group.primaryVersion.version)}`);
							else return [createTextVNode(toDisplayString(group.primaryVersion.version), 1)];
						}),
						_: 2
					}, _parent));
					...
					if (group.isExpanded && group.versions.length > 1) {
						...
						ssrRenderList(group.versions.slice(1), (v) => {
							_push(ssrRenderComponent(_component_NuxtLink, {
								...
								default: withCtx((_, _push, _parent, _scopeId) => {
									if (_push) {
										_push(`<span class="truncate"${_scopeId}>${ssrInterpolate(v.version)}</span>`);
										if (v.tags?.length) {
											_push(`<span class="flex items-center gap-1 shrink-0"${_scopeId}><!--[-->`);
											ssrRenderList(v.tags, (tag) => {
												_push(`<span class="${ssrRenderClass([tag === "latest" ? "bg-emerald-500/10 text-emerald-400" : "bg-bg-muted text-fg-subtle", "text-[9px] px-1 py-0.5 rounded font-sans font-medium"])}"${_scopeId}>${ssrInterpolate(tag)}</span>`);
											});
											_push(`<!--]--></span>`);
										} else _push(`<!---->`);
									} else return [createVNode("span", { class: "truncate" }, toDisplayString(v.version), 1), v.tags?.length ? (openBlock(), createBlock("span", {
										key: 0,
										class: "flex items-center gap-1 shrink-0"
									}, [(openBlock(true), createBlock(Fragment, null, renderList(v.tags, (tag) => {
										return openBlock(), createBlock("span", {
											key: tag,
											class: ["text-[9px] px-1 py-0.5 rounded font-sans font-medium", tag === "latest" ? "bg-emerald-500/10 text-emerald-400" : "bg-bg-muted text-fg-subtle"]
				...
				_push(ssrRenderComponent(_component_NuxtLink, {
					to: `/package/${__props.packageName}`,
...
//# sourceMappingURL=VersionSelector-BXMM0gX8.mjs.map
