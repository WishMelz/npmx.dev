import { K as createError$1, M as useFetch, R as useLazyAsyncData } from './server.mjs';
import { u as useResolvedVersion, a as useRepoMeta } from './useRepoMeta-B_K3z5Dv.mjs';
import { u as useCachedFetch, a as NPM_API } from './useCachedFetch-B1uvSFXX.mjs';
...
import { b5 as normalizeGitUrl, ab as joinURL, aM as encodePackageName } from '../nitro/nitro.mjs';
...
function usePackageDownloads(name, period = "last-week") {
	...
	return useLazyAsyncData(() => `downloads:${toValue(name)}:${toValue(period)}`, async (_nuxtApp, { signal }) => {
		const encodedName = encodePackageName(toValue(name));
		const { data, isStale } = await cachedFetch(`${NPM_API}/downloads/point/${toValue(period)}/${encodedName}`, { signal });
		return {
			...data,
			isStale
		};
...
var Package_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	async setup(__props) {
		...
		const formattedStars = computed(() => Intl.NumberFormat("en", {
			notation: "compact",
		...
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<div${ssrRenderAttrs(mergeProps({ class: "h-full w-full flex flex-col justify-center px-20 bg-[#050505] text-[#fafafa] relative overflow-hidden" }, _attrs))}><div class="relative z-10 flex flex-col gap-6"><div class="flex items-start gap-4"><div class="flex items-center justify-center w-16 h-16 p-4 rounded-xl shadow-lg bg-gradient-to-tr from-[#3b82f6]" style="${ssrRenderStyle({ backgroundColor: unref(primaryColor) })}"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m7.5 4.27 9 5.15"></path><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"></path><path d="m3.3 7 8.7 5 8.7-5"></path><path d="M12 22V12"></path></svg></div><h1 class="text-8xl font-bold tracking-tighter" style="${ssrRenderStyle({ "font-family": "'Geist Sans', sans-serif" })}"><span style="${ssrRenderStyle({ color: unref(primaryColor) })}" class="opacity-80">./</span>${ssrInterpolate(unref(pkg)?.name)}</h1></div><div class="flex items-center gap-5 text-4xl font-light text-[#a3a3a3]" style="${ssrRenderStyle({ "font-family": "'Geist Sans', sans-serif" })}"><span class="px-3 py-1 me-2 rounded-lg border font-bold opacity-90" style="${ssrRenderStyle({
...
//# sourceMappingURL=Package-B5NBQcuL.mjs.map
