...
export class FileFetcher {
  ...
  async #fetchRemoteOnce(specifier, options, httpCache) {
    ...
    if (options?.checksum != null) {
      ...
      const actualChecksum = Array.from(new Uint8Array(digest)).map((b)=>b.toString(16).padStart(2, "0")).join("");
  ...
  async fetch(specifier, // Providing a checksum here doesn't make sense because the provided
  ...
  async fetchOnce(specifier, options) {
    ...
    if (response) {
      ...
      throw new Deno.errors.PermissionDenied(`A remote specifier was requested: "${specifier.toString()}", but --no-remote is specified.`);
    } else {
...
//# sourceMappingURL=file_fetcher.js.map